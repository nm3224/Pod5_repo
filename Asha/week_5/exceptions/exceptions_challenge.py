...
print('Question 1: validate_user_input')
...
def validate_user_input():
     ...
     try:
         int(user_input)
         print(f'You have entered an integer {user_input}.')
         return user_input
     except ValueError:
         print(f'Your entry of {user_input} is not a valid number, please try again.')
         validate_user_input()
...
print('Question 2: print_tenth_item')
...
user_number = validate_user_input()
print(f'The number user entered is {user_number}.')
print()
...
def print_tenth_item(top_ten):
    if len(top_ten) < 10:
        print(f'Your list is not the correct length. It needs to be at lease 10 items. You should add {10-len(top_ten)} items to the list.')
    else:
