...
# Create nested loops to find and print all the prices
# extend(): Iterates over its argument and adding each element to the list and extending the list. The length of the list increases by number of elements in it’s argument. #Append: Adds its argument as a single element to the end of a list. The length of the list increases by one.
...
for laptop in laptops:
    for type in laptop['types']:
        all_prices.extend(type['price'])
all_princes = all_prices.sort()

print(f"Laptops are available in following price list represented in ascending order: {all_prices}")